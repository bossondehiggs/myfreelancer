class GigsController < ApplicationController
  ...
  def create
    ...
    if @gig.save
      ...
      redirect_to edit_gig_path(@gig), notice: "Guardar..."
    else
  ...
  def update
      ...
      gig_params[:pricings_attributes].each do |index, pricing|
        if @gig.has_single_pricing && pricing[:pricing_type] != Pricing.pricing_types.key(0)
          ...
          if pricing[:title].blank? || pricing[:description].blank? || pricing[:delivery_time].blank? || pricing[:price].blank?
            return redirect_to request.referrer, flash: {error: "Precio no válido"}
          end
        end
      end
    end
    ...
    if @step == 3 && gig_params[:description].blank?
      return redirect_to request.referrer, flash: {error: "La descripción no puede estar en blanco"}
    end

    if @step == 4 && @gig.photos.blank?
      return redirect_to request.referrer, flash: {error: "Necesitas subir al menos una foto"}
    end
    ...
    if @step == 5
      ...
      if @gig.description.blank?
        ...
        return redirect_to edit_gig_path(@gig, step: 4), flash: {error: "Necesitas subir al menos una foto"}
      end
    end
  ...
  def is_authorised
    redirect_to root_path, alert: "No tienes permiso.." unless current_user.id == @gig.user_id
  end
