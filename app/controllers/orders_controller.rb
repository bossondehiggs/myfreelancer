class OrdersController < ApplicationController
    ...
    def create
        ...
        if (pricing && !gig.has_single_pricing) || (pricing && pricing.basico? && gig.has_single_pricing)
            charge(gig,pricing)
        else
            flash[:alert] = "El precio es incorrecto"
        end
...
def complete
    ...
    def charge(gig, pricing)
        ...
        if order.save
            flash[:notice] = "Tu orden se ha creado correctamente"
        else
